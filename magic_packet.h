...
extern "C"
{
    ...
    /**
     * @brief Checks if the magic packet filter is currently enabled.
     *
     * @return true if the magic packet filter is enabled, false otherwise.
     */
    uint8_t isMagicPacketFilterEnabled(void);

    /**
     * @brief Gets the current configuration of the magic packet filter.
     *
     * @param enablePayload_a Pointer to a MagicPacketEnablePayload_t struct to store the current filter configuration.
     */
    void getMagicPacketFilterOptions(MagicPacketEnablePayload_t *enablePayload_a);

    /**
     * @brief Gets the default configuration of the magic packet wake-up transmit payload. Only FC is updated.
     *
     * @param wakeTxPayload_a Pointer to a MagicPacketPayload_t struct to store the current wake-up transmit payload configuration.
     */
